...
> ⚠️ **PIVOT 2026-04-29 — ver [ADR-005](../decisions/adr/ADR-005-pivot-brevidade-para-artigo-denso-blog.md):** após reunião com Marcos (sócio operacional LDC), pipeline pivotou de Brevidade Inteligente em `/news` (MDX + commit GitHub) para **artigo denso em `/blog` (Supabase BlogPost)**. Esta SPEC continua válida para os RFs que sobreviveram (RF-005, RF-006, RF-007 técnicos; RF-008 compliance; RF-015 telemetria) e está descontinuada para os RFs que viraram fora de escopo (RF-001, RF-002, RF-003, RF-004, RF-009, RF-010, RF-011, RF-012, RF-013, RF-014, RF-016 — implementação de `/news` rotas + admin + commit + digest + Telegram).
>
...
*Próximo passo: contratos Zod em `src/features/news/contracts/*.ts` + espelho legível em `docs/contracts/contracts-pipeline-ia.md`.*
