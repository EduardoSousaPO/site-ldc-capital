...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      type="button"
      onClick={(e) => {
        // Não prevenir o comportamento padrão - permite que o Select abra
        // Apenas prevenir propagação se necessário
        e.stopPropagation();
  ...
  return (
    <SelectPrimitive.Item
      ...
      onPointerDown={(e) => {
        ...
        if (nativeEvent && 'stopImmediatePropagation' in nativeEvent) {
          nativeEvent.stopImmediatePropagation();
        }
        // Chamar handler original se existir
        onPointerDown?.(e);
      ...
      className={cn(
        "focus:bg-gray-100 focus:text-gray-900 hover:bg-gray-50 relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 transition-colors",
        className
      )}
      {...props}
    >
